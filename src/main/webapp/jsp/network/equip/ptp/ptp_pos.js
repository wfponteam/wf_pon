...
NETWORK.ptp_pos = Ext.extend(Ext.Panel, {
	id 			: 'NETWORK.PtpPos',
	region 		: 'center',
	border 		: false,
	frame 		: true,
	...
	initComponent : function() {
		...
		this.initData();
		NETWORK.ptp_pos.superclass.initComponent.call(this);
	},
	//修改赋初值
	initData : function() {
		var scope = this;
		...
		if (!Ext.isEmpty(this._ptpCuid)) {
			...
			PtpManageAction.queryPtpByCuid(this._devTable,this._ptpCuid, function(_data){
				data = _data;
			});
			//查询单条数据赋值给_record
			this._record = data;
    ...
    save:function(){
		...
		if(this.showEquipmentInfoPanel.getForm().isValid()){
			...
			if(returnValue=='YES'){
				Ext.Msg.alert('提示','该POS设备下端口编号【'+portNo+'】已存在！');
				return;
			}else{
				...
				if(returnFALGE=="YES"){
					Ext.Msg.alert('提示','该POS设备下端口名称已存在！');
					return;
				}else{
...
Ext.reg('NETWORK.PtpPos', NETWORK.ptp_pos);