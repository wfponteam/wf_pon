				...
				if(xRow != null){
					...
					if(neNameStr!= null && !"".equals(neNameStr.toString().trim())
						...
						if(excelName.equals(Constant.OLTPORTNAME)){
							mapPort = verificationOltPortCell(writeWorkBook, writeSheet, xRow, headingMap, i,  lastColumns, portMaps, cardFdnMap,ptpNoMap,neMaps);
						}
		...
		try{
			...
			if(!ImportCommonMethod.isEmpty(neName)){
				...
				if(neMap!=null){
					...
					if(excelName.equals(Constant.OLTPORTNAME)){
						mapPort.put("DEV_TYPE", ElementEnum.PON_CONFIG_TYPE._olt);
						mapCard.put("DEV_TYPE", ElementEnum.PON_CONFIG_TYPE._olt);
						if(!ImportCommonMethod.isEmpty(cardNameStr)){
							if(cardMaps.containsKey(neName+cardNameStr)){
								mapCard = cardMaps.get(neName+cardNameStr);
								cardFdn = mapCard.get("FDN")==null?"":mapCard.get("FDN").toString();
								mapPort.put("RELATED_CARD_CUID", mapCard.get("CUID"));
							}else{
								List<Map> tempList = ImportCommonMethod.oltManageBO.getCardByLabelCnAndNeCuid(cardNameStr, neCuid);
								...
								Object modelObj = cardKindMap.get(cardModel);
								if(modelObj != null){
									String	modelAndCardKind = modelObj.toString();
									String[] merger = modelAndCardKind.split("\\|");
									if(merger.length==2){
										mapCard.put("MODEL",merger[0]);
										mapCard.put("CARD_KIND", merger[1]);
									}
								}
...
//					}
	...
	/**
	 * olt端口的校验, 并且获取EXCEL数据。
	 */
	public  Map verificationOltPortCell(Workbook writeWorkBook, Sheet writeSheet,Row xRow ,Map headingMap,
			int i ,int lastColumns,Map portNameMap,Map cardFdnMap,Map ptpNoMap,Map<String,Map> neMaps) throws Exception{
 		LogHome.getLog().info("OLT端口校验start====");
		//所属设备
		int relatedNeCuid = Integer.parseInt(headingMap.get(Constant.RELATEDNECUIDSB).toString());
		//板卡名称
		int cardName = Integer.parseInt(headingMap.get(Constant.CARDNAME).toString());
		//单元盘型号
		int	singeModel = Integer.parseInt(headingMap.get(Constant.SINGLEMODEL).toString());
		//所属机槽编号
		int relatedUpperComponentCuid = Integer.parseInt(headingMap.get(Constant.RELATEDUPPERCOMPONENTCUID).toString());
		//端口名称
		int portName = Integer.parseInt(headingMap.get(Constant.PORTNAME).toString());
		//端口编号
		...
		try{
		    ...
		    String cardFdn = "";
			if(!ImportCommonMethod.isEmpty(neStr)&&!ImportCommonMethod.isEmpty(cardTempFdn)&&!ImportCommonMethod.isEmpty(cardStr)){
				boolean res = ImportCommonMethod.verificationCardFdn(writeWorkBook, writeSheet, relatedUpperComponentCuid, i, lastColumns,Constant.RELATEDUPPERCOMPONENTCUID);
				...
				//校验板卡FDN信息是否重复
				if(cardSam&&fdnSam&&res&&ImportCommonMethod.isNotEmpty(neFdn)){
					String cardFdn1 = getCardPashFdn(cardTempFdn);
					String cardFdn2 = AnFdnHelper.makeOltCardFdn(neFdn,cardFdn1, "1");
					//校验板卡FDN是否已经存在
					List<Map> tempList = ImportCommonMethod.oltManageBO.
							isExistCardFdn(cardFdn2);
					if(tempList!=null&&tempList.size()>0){
						...
						if(!cardStr.equals(map2.get("LABEL_CN").toString())){
							ImportCommonMethod.printErrorInfo(writeWorkBook, writeSheet, i,
									relatedUpperComponentCuid, lastColumns,"该设备所属机槽编号的板卡于系统中已存在，请核查机槽编号！");
						}else {
							cardFdn = cardFdn2;
						}
					}else {
					...
					if(ImportCommonMethod.isNotEmpty(cardFdn)){
						...
						map.put("CARD_RELATED_UPPER_COMPONENT_CUID", "EQUIPMENT_HOLDER"
			...
			if(!ImportCommonMethod.isEmpty(ptpStr)){
				...
				ImportCommonMethod.VerificationPorNametRepeat(writeWorkBook, writeSheet, portName, i,relatedNeCuid,cardName,lastColumns,portNameMap);
			}
			...
			if(!ImportCommonMethod.isEmpty(ptpNo)){
				...
				if(!ImportCommonMethod.isEmpty(cardTempFdn)){
					map.put("SYS_NO", cardTempFdn+"-"+ptpNo);
				}
...
//			}
			...
			LogHome.getLog().info("OLT端口校验end====");
		} catch (Exception e) {
			LogHome.getLog().error("接入网-OLT端口校验出错", e);		
			throw new Exception("接入网-OLT端口校验出错："+e.getMessage());
		}
		return map;
	}
	/**
